...

{
  ...
  return GreedyOptimizer::makeNew ( name, conf, nilProps, globdat );
}
...

{
  ...
  {
    return newOptimizer ( name, conf, globdat );
  }
  else if ( isNone( obj ) )
  {
    conf.set ( name, obj );

    return newInstance<DummyOptimizer> ( name );
  }
  else
  {
    return OptimizerFactory::newInstance ( name,  conf,
                                           props, globdat );
  }
}
