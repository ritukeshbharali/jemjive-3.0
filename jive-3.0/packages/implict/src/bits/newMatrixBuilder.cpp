...

{
  ...
  Properties     params =
    ...
    MBuilderParams::newInstance ( dofs );
  ...
  {
    mbuilder = MBuilderFactory::newInstance ( name, conf, props,
                                              params, globdat );
  }
  else
  {
    mbuilder = newMBuilder ( name, conf, params, globdat );
  }
  ...
  mbuilder->configure ( props );
  mbuilder->getConfig ( conf  );
