...
#ifndef JIVE_UTIL_TABLESPARSER_H
#define JIVE_UTIL_TABLESPARSER_H
...
class TablesParser : public jem::xml::TagParser
{
 public:

  JEM_DECLARE_CLASS       ( TablesParser, jem::xml::TagParser );


  explicit                  TablesParser

    ( const Ref<MPContext>&   mpx       = nullptr,
      const Ref<ItemSet>&     items     = nullptr,
      const Ref<Dictionary>&  tables    = nullptr,
 ...
 private:
  ...
  Ref<XTable>               table_;
  Ref<Dictionary>           tables_;
  Ref<TableParser>          tableParser_;

  String                    tableName_;
  TableType_                tableType_;
  String                    nameFilter_;
