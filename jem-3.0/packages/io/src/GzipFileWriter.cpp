...
JEM_DEFINE_CLASS( jem::io::GzipFileWriter );
...
GzipFileWriter::GzipFileWriter
  ...
  ( const String&  name,
    ...
    Super (
      newInstance<GzipOutputStream> (
        newInstance<FileOutputStream> (
          name, flags & ~FileFlags::APPEND )
      )
    )
...
GzipFileWriter::GzipFileWriter ( Ref<FileStream> fs ) :

  Super (
    newInstance<GzipOutputStream> (
      newInstance<FileOutputStream> ( fs )
    )
  )
