...
/*
  ...
  Table of CRCs of all 8-bit messages. This table has been generated
  ...
  Note that the table has been pre-computed to avoid nasty race
  conditions in multi-threaded applications.

*/


static const ulint  CRC_TABLE_[256] =

  { 0000000000UL, 0x77073096UL, 0xee0e612cUL, 0x990951baUL,
...
void updateCRC ( ulint& crc, const void* buf, idx_t len )
{
...
{
  buf[3] = (byte) (num & 255UL);
  num    = num >> 8;
  buf[2] = (byte) (num & 255UL);
  num    = num >> 8;
  buf[1] = (byte) (num & 255UL);
  num    = num >> 8;
  buf[0] = (byte) (num & 255UL);
}
...
int getWidthOf ( lint num ) noexcept
{
  lint  k = 1;
  int   w = 1;
